...
const SYSTEM_ADMIN_EMAILS = SA_EMAILS_STR.split(',')
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={title} size="max-w-md">
            ...
            <div className="flex justify-end space-x-3">
                ...
                <Button onClick={onConfirm} className="bg-red-500 hover:bg-red-600" icon={AlertTriangle}>
                    {confirmText}
                </Button>
            </div>
        </Modal>
...
const Button = ({ onClick, children, className = 'bg-blue-500 hover:bg-blue-600', icon: Icon, disabled = false, type = "button" }) => (
    <button
        type={type}
        onClick={onClick}
        disabled={disabled}
...
const InputField = ({ label, type = 'text', value, onChange, placeholder, required = false, name, min, max }) => (
    <div className="mb-4">
...
const TextAreaField = ({ label, value, onChange, placeholder, name, rows = 3 }) => (
    <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-1">{label}</label>
        <textarea
            name={name}
...
const migrateDataToFamily = async (familyId, saUserId, saDisplayName, reportErrorFunc) => {
    ...
    for (const collectionName of collectionsToMigrate) {
        ...
        try {
            ...
            oldDocsSnap.docs.forEach(oldDoc => {
                const newDocRef = doc(collection(db, newBasePath));
                let dataToMigrate = oldDoc.data();
                if (collectionName === 'kids') {
                    dataToMigrate.familyId = familyId;
                    dataToMigrate.totalEarnedPoints = dataToMigrate.totalEarnedPoints || 0; /* Initialize if missing */
                }
...
function App() {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            ...
            if (user && !user.isAnonymous) {
                ...
                const isSameUserAndSession = loggedInUser &&
                    loggedInUser.uid === user.uid &&
                    firebaseAuthUser &&
                    firebaseAuthUser.uid === user.uid;

                if (isSameUserAndSession) {
                ...
                if (userProfileData.familyRoles && userProfileData.familyRoles.length > 0) {
                    ...
                    if (userProfileData.activeFamilyRole) {
                        ...
                        const isSameUserAndSessionForPreference = loggedInUser &&
                            loggedInUser.uid === user.uid &&
                            firebaseAuthUser &&
                            firebaseAuthUser.uid === user.uid;
                        const noExistingPreference = storedProfileData.activeFamilyRole === null ||
                            storedProfileData.activeFamilyRole === undefined;
                        const isLikelyNewSessionOrNoExistingPreference = !isSameUserAndSessionForPreference || noExistingPreference;
                ...
                if (!userProfileData.isSA && user.email && (!userProfileData.familyRoles || userProfileData.familyRoles.length === 0)) {
                    ...
                    const allCurrentFamiliesList = allFamiliesForSA.length > 0
                        ? allFamiliesForSA
                        : (await getDocs(collection(db, familiesCollectionPath))).docs.map(d => ({ id: d.id, ...d.data() }));
        ...
        return () => unsubscribe();
    }, [allFamiliesForSA, firebaseAuthUser]); // Added firebaseAuthUser, consistent with original
    ...
    useEffect(() => {
        ...
        if (loggedInUserIsSA && !activeFamilyId) {
            console.log('[Effect for loggedInUser] SA in Admin Dashboard view. Fetching all families.');
        ...
        } else if (loggedInUserUid && !activeFamilyId && !loggedInUserIsSA) {
    ...
    }, [isAuthReady, firebaseAuthUser, loggedInUserUid, loggedInUserIsSA, activeFamilyId]);
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
                ...
                <Button
                    onClick={() => { setErrorState(''); window.location.reload(); }}
            ...
            {families.length === 0 ? <p className="text-gray-500">No families created yet.</p> : (
                <ul className="space-y-3">
                    {families.map(fam => (
                        <li
                            key={fam.id}
                            className="flex flex-col sm:flex-row justify-between sm:items-center p-4 bg-gray-50 rounded-lg shadow-sm"
                        >
                            <div>
            ...
            {selectedFamilyId && (
                <div className="mt-6">
                    <h4 className="text-lg font-medium text-gray-600">
                        Current Parents in {families.find(f => f.id === selectedFamilyId)?.familyName || ''}:
                    </h4>
                    <p className="text-sm text-gray-500">
                        (Displaying this list requires querying the 'users' collection based on familyRoles - TBD)
                    </p>
                </div>
            )}
        </Card>
...
const ManageKids = ({ parentUser, familyId, kidsInFamily, completedTasks, showConfirmation }) => {
                        ...
                        if (kidUserDoc.exists() && !kidUserDoc.data().familyRoles?.find(fr => fr.familyId === familyId && fr.role === 'kid')) {
                            await updateDoc(kidUserDocRef, {
                                familyRoles: arrayUnion({
                                    familyId: familyId,
                                    role: 'kid',
                                    familyName: parentUser.activeFamilyRole.familyName
                                })
                            });
                        }
                    }
    ...
    return (
        <Card>
            ...
            {kidsInFamily.length === 0 ? (
                <p className="text-gray-500">No kids added to this family yet.</p>
            ) : (
                <ul className="space-y-3">
                    {kidsInFamily.map(kid => {
                        const pendingPoints = completedTasks
                            ...
                            >
                                <div>
                                    ...
                                    <div className="text-sm space-x-2">
                                        ...
                                        {pendingPoints > 0 && (
                                            <span className="text-yellow-600 font-semibold">Pending: {pendingPoints} pts</span>
                                        )}
                                    </div>
                                    {kid.email && (
                                        <p className="text-xs text-gray-500 mt-1 flex items-center">
                                            <Mail size={12} className="mr-1" /> {kid.email}
                                        </p>
                                    )}
            ...
            >
                ...
                <p className="text-xs text-gray-500 mb-3">
                    If email is provided, the kid can log in using Google. They need to log in once for the system to link their account.
                </p>
                <Button onClick={handleSaveKid} className="w-full bg-green-500 hover:bg-green-600">
...
const ManageTasks = ({ familyId, tasksInFamily, kidsInFamily, completedTasks, showConfirmation }) => {
    ...
    const handlePointChange = (amount) => {
        setFormData(prev => ({
            ...prev,
            points: Math.max(1, (parseInt(prev.points) || 1) + amount).toString()
        }));
    };
    ...
    const handleDeleteTask = async (taskId) => {
        ...
        try {
            ...
            setTaskToDelete(null); // Ensure it's cleared even on error
        }
    };
                ...
                <ul className="space-y-3">
                    {processedAndSortedTasks.map(task => {
                        ...
                        if (task.recurrenceType === 'weekly' && task.daysOfWeek?.length > 0) {
                            recurrenceDisplay = `Weekly (${task.daysOfWeek.map(d => DAY_LABELS_SHORT[DAYS_OF_WEEK.indexOf(d)] || d).join(', ')})`;
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingTask ? "Edit Task" : "Add New Task"} size="max-w-lg">
                ...
                <div className="mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <input
                            type="number" name="points" value={formData.points} onChange={handleInputChange}
                            placeholder="e.g., 10" required min="1"
                            className="w-20 text-center px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                        />
...
const ManageRewards = ({ familyId, rewardsInFamily, showConfirmation }) => {
    ...
    const handleDeleteReward = async (rewardId) => {
        ...
        try {
            ...
            setRewardToDelete(null); // Ensure it's cleared even on error
        }
    };
    ...
    const requestSort = (key) => {
        ...
        if (sortConfig.key === key && sortConfig.direction === 'ascending') {
            ...
            direction = 'ascending'; // Toggle back to ascending
        }
        setSortConfig({ key, direction });
    };
...
const ApproveTasks = ({ familyId, pendingTasks, kidsInFamily, allTasksInFamily, showConfirmation, firebaseUser }) => {
    ...
    const openApprovalModal = (completedTask) => {
        ...
        setApprovalNote('');
        setReopenTaskFlag(true); // Default to true when opening modal
    };
    const closeApprovalModal = () => {
        setCurrentTaskForApproval(null);
        setApprovalNote('');
        setPointsToAdjust(0);
        setReopenTaskFlag(false); // Reset for next time
    };

    const handleConfirmApprovalAction = (status) => {
        const actionText = status === 'approved'
            ? 'approve'
            : (reopenTaskFlag ? 'reject and reopen' : 'reject');
        const title = status === 'approved' ? 'Confirm Approval' : 'Confirm Rejection';
        showConfirmation(
            title,
            `Are you sure you want to ${actionText} this task submission for "${currentTaskForApproval?.taskName}"?`,
            () => processApprovalAction(status)
        );
    };
    ...
    const processApprovalAction = async (status) => {
        ...
        try {
            ...
            if (!kidDoc.exists() || !mainTaskDoc.exists()) {
                console.error("Kid or Task document not found"); return;
            }
            ...
            // Potentially set an error message to UI
        }
    };
            ...
            <Modal isOpen={!!currentTaskForApproval} onClose={closeApprovalModal} title={`Review Task: ${currentTaskForApproval?.taskName}`}>
                {currentTaskForApproval && (
                    <div>
                        ...
                        <div className="my-4">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <input
                                    ...
                                    onChange={e => setPointsToAdjust(Math.max(0, parseInt(e.target.value) || 0))}
                        ...
                        <TextAreaField
                            label="Approval/Rejection Note (Optional)"
                            value={approvalNote}
                        ...
                        <div className="mt-4 mb-2">
                            <label className="flex items-center text-sm text-gray-600">
                                <input
                                    type="checkbox"
                                    checked={reopenTaskFlag}
                                    onChange={(e) => setReopenTaskFlag(e.target.checked)}
                        ...
                        <div className="flex justify-end space-x-3 mt-6">
                            <Button onClick={() => handleConfirmApprovalAction('rejected')} className="bg-red-500 hover:bg-red-600" icon={ThumbsDown}>
                                {reopenTaskFlag ? "Reject & Reopen" : "Reject Only"}
                            </Button>
                            <Button onClick={() => handleConfirmApprovalAction('approved')} className="bg-green-500 hover:bg-green-600" icon={ThumbsUp}>
                                Approve
                            </Button>
                        </div>
                    </div>
    ...
    if (pendingRewards.length === 0) {
        return (
            <Card>
                ...
                <p className="text-gray-500">No rewards pending fulfillment.</p>
            </Card>
        );
    }
        ...
        <Card>
            ...
            <ul className="space-y-4">
                {pendingRewards.map(rr => {
                    ...
                    const rewardInfo = allRewardsList.find(r => r.id === rr.rewardId);
                    return (
                                ...
                                <div>
                                    ...
                                    {rewardInfo && (
                                        <p className="text-xs text-gray-400 italic">
                                            Original reward ID (in family): {rewardInfo.id}
                                        </p>
                                    )}
                                </div>
                                <div className="flex space-x-2 mt-2 sm:mt-0">
        ...
        <Card>
            ...
            <div className="mb-8">
                ...
                {filteredRedeemed.length === 0 ? (
                    <p className="text-gray-500">No rewards activity for this period.</p>
                ) : (
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredRedeemed.map(rr => {
                                    ...
                                    let statusClass = '';
                                    if (rr.status === 'cancelled_by_parent') statusClass = 'bg-red-50 text-red-700';
                                    else if (rr.status === 'fulfilled') statusClass = 'bg-green-50 text-green-700';
                                    else if (rr.status === 'pending_fulfillment') statusClass = 'bg-yellow-50 text-yellow-700';
                                    ...
                                    return (
                                        <tr key={rr.id} className={statusClass.split(' ')[0] /* only background part */}>
                                            ...
                                            <td className={`px-4 py-2 whitespace-nowrap text-sm font-medium ${statusClass.split(' ')[1] || 'text-gray-700'}`}>
                                                {rr.status?.replace(/_/g, ' ') || 'Pending'}
                                            </td>
            ...
            <div>
                ...
                {filteredCompleted.length === 0 ? (
                    <p className="text-gray-500">No tasks submitted/reviewed for this period.</p>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                ...
                                {filteredCompleted.map(ct => {
                                    ...
                                    return (
                                        <tr key={ct.id} className={statusBgClass}>
                                            ...
                                            <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate" title={ct.approvalNote}>
                                                {ct.approvalNote || '-'}
                                            </td>
                                        </tr>
                                    );
...
const KidTasksList = ({ kid, familyId, allTasks, completedTasks, showConfirmation }) => {
    ...
    const handleCompleteTask = async (task) => {
        ...
        if (!taskDueDateForCompletion) {
            setShowFeedback(`Cannot determine due date for "${task.name}". Contact parent.`);
            setFeedbackType('error');
            setTimeout(() => setShowFeedback(''), 4000);
            return;
        }

        // Check if already submitted for this *specific* due date
        const alreadySubmittedForThisDueDate = completedTasks.find(ct =>
            ct.taskId === task.id &&
            ct.kidId === kid.id &&
            ct.status === 'pending_approval' &&
            ct.taskDueDate?.toDate().getTime() === taskDueDateForCompletion.toDate().getTime()
        );

        if (alreadySubmittedForThisDueDate) {
            setShowFeedback(`You've already submitted "${task.name}" for this due date. Waiting for approval.`);
        ...
        try {
            await addDoc(collection(db, getFamilyScopedCollectionPath(familyId, 'completedTasks')), {
                ...
                taskDueDate: taskDueDateForCompletion, // Store the specific due date of this completion
    ...
    const handleWithdrawTask = async (completedTaskId) => {
        ...
        try {
            ...
            setFeedbackType('info');
            setTaskToWithdraw(null); // Clear selection
        } catch (error) {
            console.error("Error withdrawing task: ", error);
            setShowFeedback("Failed to withdraw task. Please try again.");
            setFeedbackType('error');
        } finally {
            setTimeout(() => setShowFeedback(''), 3000);
        }
...
const KidRewardsList = ({ kid, familyId, rewards, showConfirmation }) => {
    ...
    const confirmRedeemReward = (reward) => {
        if ((kid.points || 0) < reward.pointCost) {
            setShowFeedback("You don't have enough points for this reward.");
            setFeedbackType('error');
            setTimeout(() => setShowFeedback(''), 3000);
            return;
        }
        setRewardToRedeem(reward); // Set the full reward object
        showConfirmation(
            "Confirm Redemption",
            `Redeem "${reward.name}" for ${reward.pointCost} points?`,
            handleRedeemReward, // This will be called by the confirmation modal
            "Redeem"
        );
    };

    const handleRedeemReward = async () => {
        if (!rewardToRedeem) return; // Should have the reward object here

        // Double check points just before committing
        if ((kid.points || 0) < rewardToRedeem.pointCost) {
            setShowFeedback("Not enough points (checked again). Please earn more!");
            setFeedbackType('error');
            setTimeout(() => setShowFeedback(''), 3000);
            setRewardToRedeem(null); // Clear selection
            return;
        }
        ...
        try {
            ...
            // Create a redeemedReward document
            ...
            batch.set(newRedeemedRewardRef, {
                kidId: kid.id,
                kidName: kid.name, // Store kid's name at time of redemption
                rewardId: rewardToRedeem.id,
                rewardName: rewardToRedeem.name, // Store reward's name at time of redemption
            ...
            setShowFeedback(`"${rewardToRedeem.name}" redeemed! Your parent will fulfill it soon.`);
            setFeedbackType('success');

        } catch (error) {
            console.error("Error redeeming reward: ", error);
            setShowFeedback('Oops! Something went wrong while redeeming. Please try again.');
            setFeedbackType('error');
        } finally {
            setRewardToRedeem(null); // Clear selection
            setTimeout(() => setShowFeedback(''), 4000);
...
const KidHistory = ({ kid, familyId, completedTasks, redeemedRewards }) => {
    ...
    const filteredCompletedTasks = filterData(kidCompletedTasks, 'dateSubmitted')
        .sort((a, b) => b.dateSubmitted.toMillis() - a.dateSubmitted.toMillis());
    const filteredRedeemedRewards = filterData(kidRedeemedRewards, 'dateRedeemed')
        .sort((a, b) => b.dateRedeemed.toMillis() - a.dateRedeemed.toMillis());
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-end mb-0 -mt-4"> {/* Adjust margins if needed */}
                <SelectField
                    value={filterPeriod}
                    onChange={e => setFilterPeriod(e.target.value)}
                    options={[
                            ...
                            <li key={ct.id} className={`p-3 rounded-lg shadow-sm ${getStatusBgClass(ct.status)}`}>
                                ...
                                <p className="text-sm text-gray-500">
                                    Status: <span className={`font-semibold ${getStatusClass(ct.status)}`}>
                                        {ct.status?.replace(/_/g, ' ') || 'Pending'}
                                    </span>
                                    {ct.status === 'approved' && ` (+${ct.pointsAwarded || ct.taskPoints || 0} pts)`}
                                </p>
