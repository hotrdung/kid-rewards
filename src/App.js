                ...
                <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-2 sm:space-x-4">
                        ...
                        {!loggedInUser.isSA && loggedInUser.activeFamilyRole?.role === 'kid' && (
                            <span className="px-3 py-1 text-sm font-semibold rounded-full bg-green-100 text-green-700">Kid</span>
                        )}
            ...
            <nav className="bg-white shadow rounded-lg p-2">
                <div className="flex flex-wrap gap-1 sm:gap-2">
                    {adminNavItems.map(item => (
                        <div key={item.name} className="flex-1 min-w-[120px] sm:min-w-0"> {/* Adjusted min-width for typically longer SA labels */}
                            <NavItemButton
                                tabName={item.name}
                                icon={item.icon}
                                label={item.label}
...
const ManageFamilies = ({ families, showConfirmation, currentUser }) => {
    ...
    const openAddModal = () => { setEditingFamily(null); setFamilyName(''); setFormError(''); setIsModalOpen(true); };
    const openEditModal = (family) => { setEditingFamily(family); setFamilyName(family.familyName); setFormError(''); setIsModalOpen(true); };
    const handleSaveFamily = async () => { if (!familyName.trim()) { setFormError('Family name is required.'); return; } setFormError(''); const familyData = { familyName: familyName.trim(), updatedAt: Timestamp.now(), updatedBy: currentUser.uid, }; try { if (editingFamily) { await updateDoc(doc(db, familiesCollectionPath, editingFamily.id), familyData); } else { familyData.createdAt = Timestamp.now(); familyData.createdBy = currentUser.uid; await addDoc(collection(db, familiesCollectionPath), familyData); } setIsModalOpen(false); } catch (e) { console.error("Error saving family:", e); setFormError("Failed to save family."); } };
    const confirmDeleteFamily = (family) => { showConfirmation("Delete Family", `Are you sure you want to delete the family "${family.familyName}"? This will delete ALL associated data (kids, tasks, rewards, history) and cannot be undone. THIS IS A PLACEHOLDER - FULL DATA DELETION REQUIRES SERVER-SIDE LOGIC.`, async () => { console.warn("Attempting to delete family doc. Cascading delete of subcollections needs server-side implementation."); try { await deleteDoc(doc(db, familiesCollectionPath, family.id)); } catch (e) { console.error("Error deleting family doc:", e); } }); };
    ...
    return (
        <Card>
            ...
            {families.length === 0 ? <p>No families created yet.</p> : (
                <ul className="space-y-3">
                    {families.map(fam => (
                        <li key={fam.id} className="flex justify-between items-center p-3 bg-gray-50 rounded">
                            <span>{fam.familyName} <span className="text-xs text-gray-400">({fam.id})</span></span>
                            <div className="space-x-2">
                                <Button onClick={() => openEditModal(fam)} icon={Edit3} className="bg-blue-500 hover:bg-blue-600 text-sm px-2 py-1" title="Edit Family">
                                    <span className="hidden sm:inline ml-1">Edit</span>
                                </Button>
                                <Button onClick={() => confirmDeleteFamily(fam)} icon={Trash2} className="bg-red-500 hover:bg-red-600 text-sm px-2 py-1" title="Delete Family">
...
const ParentRewardHistory = ({ familyId, redeemedRewards, completedTasks, kidsInFamily, rewardsInFamily, tasksInFamily }) => {
    const [filterPeriod, setFilterPeriod] = useState('all'); const now = new Date(); const filterData = (data, dateField) => {
        if (filterPeriod === 'all') return data; return data.filter(item => {
            const itemDate = item[dateField]?.toDate(); if (!itemDate) return false;
    ...
    const handleCompleteTask = async (task) => {
        ...
        try {
            await addDoc(collection(db, getFamilyScopedCollectionPath(familyId, 'completedTasks')), {
                ...
                taskDueDate: taskDueDateForCompletion,
